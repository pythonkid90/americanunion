...
- A full REST API (documented in the GitHub wiki) to view and edit nation stats directly, with GET, POST, and DELETE requests. There is also a password-protected internal endpoint that allows raw unfiltered edits as well as a map upload and backup endpoint.
...
#### Low Priority
- Some more error handling when a faulty input is given to the REST API (the website itself seems safe already).
